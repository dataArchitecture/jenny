...
class Kiwoom(QAxWidget):
	def __init__(self):
		...
		self.db_handling = stock_db()

		# 데이터베이스 연결
		result_xx = self.db_handling.db_select('select * from test_table limit 10')
		print('SQL 테스트')
		print(result_xx)
		...
		self.screen_meme_stock = "6000" # 종목별로 할당할 주문 스크린 번호
		########################
		...
		self.portfolio_stock_dict = {} ## 투자 후보
		self.jango_dict = {} ## 장고 주식
		########################
		...
		self.detail_account_info() ## 예수금가져오는코드
	...
	def real_event_slots(self):
		...
		self.OnReceiveChejanData.connect(self.chejan_slot) ## 주문에 대한 이벤트, 체결/잔고
	...
	def get_account_info(self):
		...
		print("나의 보유 계좌번호 %s" % self.account_num) # 8145748711
	...
	def detail_account_info(self):
		...
		self.dynamicCall("SetInputValue(QString, QString)", "조회구분", "2") # 일반조회
	...
	def detail_account_mystock(self, sPreNext="0"):
		print("계좌평가잔고내역요청을 요청하는 부분 / 연속조회 %s" % sPreNext)

		self.dynamicCall("SetInputValue(QString, QString)", "계좌번호", self.account_num)
		self.dynamicCall("SetInputValue(QString, QString)", "비밀번호", "0000")
		self.dynamicCall("SetInputValue(QString, QString)", "비밀번호입력매체구분", "00")
		self.dynamicCall("SetInputValue(QString, QString)", "조회구분", "2")  # 일반조회
		self.dynamicCall("CommRqData(QString, QString, int, QString)", "계좌평가잔고내역요청", "opw00018", sPreNext, self.screen_my_info)
	...
	def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
		...
		if sRQName == "예수금상세현황요청":
			...
			for i in range(rows):
				...
				learn_rate = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "수익률(%)")
			...
			for i in range(rows):
				...
				order_status = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "주문상태") # 접수 -> 확인 -> 체결
				...
				order_gubun = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "주문구분") # -매도, +매수,
			...
			print("데이터 일수 %s" % cnt) ## 한번 조회시 600일치 데이터를 가져옴

			#GetCommDataEx : 전체 가져오는 방식이 있음
			# data = self.dynamicCall("GetCommDataEx(QString, QString)", sTrCode, sRQName)
			# [[ '', '현재가', '거래량', '거래대금', '일자', '시가', '고가', '저가', ''], ['', '현재가', '거래량', ....]]

			for i in range(cnt):
				data = []

				current_price = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "현재가")
				value = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "거래량")
				trading_value = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "거래대금")
				date = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "일자")
				start_price = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "시가")
				high_price = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "고가")
				low_price = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, i, "저가")
			...
			if sPrevNext == "2":
				...
				if self.calcul_data == None or len(self.calcul_data) < 120:
					...
					check_price = None
					if int(self.calcul_data[0][7]) <= moving_avg_price and moving_avg_price <= int(self.calcul_data[0][6]):
						print("오늘 주가가 걸쳐 있는 것 확인")
						bottom_stock_price = True
						check_price = int(self.calcul_data[0][6])

					#과거 일봉들이 120일 이평선보다 밑에 있는지 확인.
					#확인중에 일봉이 120일 이평선보다 위에 있으면 계산진행
					...
					if bottom_stock_price == True:
						...
						while True:
							...
							if moving_avg_price_prev <= int(self.calcul_data[idx][6]) and idx <= 20:
								print("20일 동안 주가가 120일 이평선과 같거나 위에 있으면 통과 못 함")
								price_top_moving = False
								break
							elif int(self.calcul_data[idx][7]) > moving_avg_price_prev and idx > 20:
	...
	def day_kiwoom_db(self, code=None, date=None, sPrevNext="0"):
		...
		self.dynamicCall("CommRqData(QString, QString, int, QString)", "주식일봉차트조회요청", "opt10081", sPrevNext, self.screen_calculation_stock) # TR 서버로 전송 - Transaction
	...
	def realdata_slot(self, sCode, sRealType, sRealData):
		if sRealType == "장시작시간":
			...
			if value == '0':
				...
				for code in self.portfolio_stock_dict.keys():
					self.dynamicCall("SetRealRemove(QString, QString)", self.portfolio_stock_dict[sCode]['스크린번호'], code)
			...
			c = abs(int(c))
			d = self.dynamicCall("GetCommRealDAta(QString, int)", sCode, self.realType.REALTYPE[sRealType]['등락율']) # +(-) 50
			d = float(c)
			...
			# 계좌 잔고 평개 나역에 있고, 오늘 산 잔고에는 없을 경우
			...
			# 등락율이 2.0% 이상이고 오늘 산 잔고에 없을 경우
				...
				order_success = self.dynamicCall(
					...
					, ["신규매수", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.account_num, 1
						, sCode, quantity, 0, self.realType.SENDTYPE['거래구분']['지정가'], ""])
			...
			not_meme_list = list(self.not_account_stock_dict) ## self.not_account_stock_dict.copy()
	...
	def chejan_slot(self, sGubun, nItemCnt, sFIdList):
		if int(sGubun) == 0:
			...
			order_quan = int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문수량']))
			order_price = int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문가격']))
			not_chegual_quan = int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['미체결수량']))
			order_gubun = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문구분']) ## 출력 : -매도, +매도
			order_gubun = order_gubun.strip().lstrip('+').lstrip('-')

			chegual_time_str = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문/체결시간']) ## 매수 : 2, 매도 : 1
			chegual_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['체결가'])
			chegual_price = 0 if chegual_price == 0 else int(chegual_price)
			chegual_quantity = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['체결량'])
			chegual_quantity = 0 if chegual_quantity == 0 else int(chegual_quantity)

			current_price = abs(int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['현재가'])))
			first_sell_price = abs(int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['(최우선)매도호가'])))
			first_buy_price = abs(int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['(최우선)매수호가'])))

			## 새로 들어온 주문이면 주문번호 할당
			if order_number not in self.not_account_stock_dict.keys():
				code = self.not_account_stock_dict.update({order_number: {}})
			...
			stock_name = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['종목명']).strip()

			current_price = abs(int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['현재가'])))
			stock_quan = int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['보유수량']))
			like_quan = int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['주문가능수량']))
			buy_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['매입단가'])
			total_buy_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['총매입가'])

			meme_gubun = self.realType.REALTYPE['매도수구분'][self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['매도매수구분'])]
#			order_gubun = int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['주문구분'])) # 출력:-매도, +매도
			first_sell_price = abs(int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['(최우선)매도호가'])))
			first_buy_price = abs(int(self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['(최우선)매수호가'])))

			## 새로 들어온 주문이면 종목코드 할당
			if sCode not in self.jango_dict.keys():
				code = self.jango_dict.update({sCode: {}})
