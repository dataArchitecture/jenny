# 원본 URL : https://nowonbun.tistory.com/678
...
class stock_db():
	def __init__(self):
		...
		self.dbconn = mysql.connector.connect(host='localhost', port=4620, user='root', passwd='jenyrichman',database='richman')

	# 검색을 할 경우 사용되는 함수.
	def db_select(self, query, bufferd=True):
	...
	def db_merge(self, query, values, bufferd=True):
		# 전역에 선언되어 있는 connection을 가져온다.
		try:
			# 커서를 취득한다.
			cursor = self.dbconn.cursor(buffered=bufferd);
	...
	def db_merge_bulk(self, query, values, bufferd=True):
		# 전역에 선언되어 있는 connection을 가져온다.
		try:
			# 커서를 취득한다.
			cursor = self.dbconn.cursor(buffered=bufferd);
	...
	# DML이외의 쿼리를 실행하는 함수.
	def db_execute(self, query, bufferd=True):
		# 전역에 선언되어 있는 connection을 가져온다.
		try:
			# 커서를 취득한다.
			cursor = self.dbconn.cursor(buffered=bufferd);
